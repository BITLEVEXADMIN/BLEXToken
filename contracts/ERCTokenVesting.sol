 ...
 *
 */
abstract contract ERCTokenVesting is Ownable, ERC20Burnable {
    ...
    */
    function _computeReleasableAmount(VestingSchedule storage vestingSchedule) internal view returns(uint256){
        ...
        if (currentTime >= vestingSchedule.start.add(vestingSchedule.duration)) {
            ...
            uint256 tgeAmount = vestingSchedule.tgeAmount;
            ...
            uint256 tgeVestedAmount = vestedAmount.add(tgeAmount);
            tgeVestedAmount = tgeVestedAmount.sub(vestingSchedule.released);
